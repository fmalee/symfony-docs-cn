...
与设置环境部变量的方式无关，你可能需要使用 ``--env-vars`` 选项运行 ``debug:container``
命令，以验证它们是否已定义且具有预期值：
...
.. code-block:: terminal
    ...
    # 运行此命令以显示一个特定环境变量的所有详细信息：
    $ php bin/console debug:container --env-var=FOO

.. versionadded:: 4.3

    Symfony 4.3中引入了调试环境变量的选项。
...
Symfony不是在shell或Web服务器中定义环境变量，而是基于位于项目根目录的名为 ``.env``
（带点号前缀）的文件，提出了一种在本地计算机中定义它们的便捷方法。
