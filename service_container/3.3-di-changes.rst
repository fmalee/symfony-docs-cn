...
而你总是可以在 ``services.yaml`` 中将 ``autoconfigure`` 设置为 ``false``，或为一个特定服务而禁用它。
...
你已经 *明确* 配置了所有服务。所以，``autowire`` 什么都不做。
你还标记了你的服务，因此 ``autoconfigure`` 也不会更改任何现有服务。
...
但是，由于旧服务ID在单独文件（``legacy_aliases.yaml``）中的是别名，因此它们
*仍然* 是公有的。这可以确保应用继续工作。
...
首先，更新你的应用以 *不* 使用旧的服务ID（既 ``legacy_aliases.yaml``）。
...
执行此操作后，你可以删除 ``legacy_aliases.yaml`` 并删除其导入。
